<template>
  <div :class="{ 'no-webp': isSafari }">
    <nav-bar @onShowLogin="showLogin = true" />
    <login-user v-if="showLogin" @close="showLogin = false" />
    <post-modal v-if="showPostModal" :post="post" />
    <info-user v-show="showUserModal" :userId="userId" />
...
export default {
  components: {
    ...
    InfoUser,
    PostModal: () =>
      import(/* webpackPrefetch: true */ "@/components/posts/PostModal.vue"),
    AppFooter,
    CookieLaw,
    NotificationModal: () =>
      import(
        /* webpackPrefetch: true */ "@/components/notifications/NotificationModal.vue"
      )
  },
  name: "app",
  created() {
